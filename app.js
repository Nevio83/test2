...
function triggerWishlistButtonAnimation(productId) {
  ...
  const wishlistButtons = document.querySelectorAll(
    `.lumiere-wishlist-btn[data-product-id="${productId}"]`,
  );
...
function deepDiagnosis() {
  ...
  const workingButton = document.querySelector(
...
function filterProducts(products, searchText, category) {
  ...
  const filtered = products.filter((product) => {
    ...
    if (
      ...
      product.category === "Technik/Gadgets"
    ) {
      console.log(
        `âœ… Technik product: ${product.name}, SKU: ${product.sku}, isNotAli: ${isNotAliExpress}`,
      );
    }
...
function createCustomScrollbarForGrid(gridId) {
  ...
  scrollbarTrack.addEventListener("click", (e) => {
    ...
    const thumbCenterX = Math.max(
      thumbWidth / 2,
      Math.min(trackWidth - thumbWidth / 2, clickX),
    );
    ...
    const scrollPercentage =
      maxThumbPosition > 0 ? thumbLeftX / maxThumbPosition : 0;
  ...
  function handleMove(clientX, event) {
    if (isDragging) {
      ...
      const newThumbLeft = Math.max(
        0,
        Math.min(maxThumbPosition, startThumbLeft + deltaX),
      );
      ...
      console.log(
        `ðŸŽ¯ Thumb dragging: thumbPos=${newThumbLeft}px, scrollPos=${newScrollLeft}px`,
      );
    }
  }
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    if (isTrackDragging) {
      ...
      const newScrollLeft = Math.max(
        ...
        Math.min(
          maxScrollLeft,
          trackStartScrollLeft + deltaX * scrollSensitivity,
        ),
      );
...
function initializeCategoryNavigation() {
  ...
  categoryTabs.forEach((tab, index) => {
    ...
    tab.addEventListener("click", (e) => {
      ...
      const allProductsTitle = document.querySelector(
        '.category-header[style*="margin-top: 3rem"] h2.category-title',
      );
      if (allProductsTitle) {
        allProductsTitle.textContent =
          category === "alle" ? "Alle Produkte" : categoryName;
        console.log(
          "âœ… Updated section title to:",
          allProductsTitle.textContent,
        );
      }
      ...
      if (category === "alle") {
        ...
        const scrollTarget = document.querySelector(
          '.category-header[style*="margin-top: 3rem"]',
        );
        if (scrollTarget) {
        ...
        if (targetGridId) {
          const targetSection = document
            .getElementById(targetGridId)
            ?.closest(".product-category-section");
          if (targetSection) {
            ...
            console.error(
              "âŒ Target section not found for category:",
              category,
            );
          }
        } else {
...
function showCategorySections(selectedCategory) {
  ...
  const bestsellerSection = document
    .querySelector("#bestsellerGrid")
    ?.closest(".product-category-section");

  // Get the "Alle Produkte Grid" section (should always stay visible)
  const alleProduktGridSection = document.querySelector(
    ".alle-produkte-grid-section",
  );

  // Get the "Alle Produkte" header section
  const alleProduktHeader = document.querySelector(
    '.category-header[style*="margin-top: 3rem"]',
  );
...
document.addEventListener("DOMContentLoaded", () => {
      ...
      .then((products) => {
        ...
        categories.forEach((category) => {
          const categoryProducts = products
            .filter((p) => p.category === category && p.showInSlider !== false)
            .slice(0, 2);
        ...
        if (bestsellerProducts.length < 6) {
          const remaining = products
            .filter(
              (p) =>
                !bestsellerProducts.includes(p) && p.showInSlider !== false,
            )
            .slice(0, 6 - bestsellerProducts.length);
        ...
        console.log(
          "ðŸ“¦ Loading bestseller products with variety:",
          finalBestsellers.length,
        );
        ...
        categoryTitles.forEach((title, index) => {
          if (index === 1) {
            // Das zweite Element ist "Alle Produkte"
  ...
  if (priceSort)
    priceSort.addEventListener("change", debounce(updateFilters, 50));
...
function optimizeImages() {
  ...
  images.forEach((img) => {
    ...
    img.addEventListener("error", function () {
      ...
      if (
        this.src &&
        !this.src.includes("data:") &&
        !this.src.includes("blob:")
      ) {
...
function initializeCategoryTiles(products) {
  ...
  categoryTiles.forEach((tile, index) => {
    ...
    newTile.addEventListener("click", (e) => {
      ...
      // Update body class for category-specific styling
      const categoryForClass =
        category === "Alle Kategorien" ? "alle" : category;
      ...
      if (categoryTitle) {
        categoryTitle.textContent =
          category === "Alle Kategorien" ? "Alle Produkte" : category;
      }
...
function scrollCategory(gridId, direction) {
  ...
  if (direction === "left") {
    ...
    if (isAtEnd) {
      ...
      else if (gridId === "wellnessGrid")
        categoryName = "Körperpflege/Wellness";
...
function initializeCategoryProducts() {
  ...
  if (window.allProducts && window.allProducts.length > 0) {
    ...
    if (categoryInitAttempts < maxCategoryInitAttempts) {
      console.log(
        `â³ Waiting for products to load... (attempt ${categoryInitAttempts}/${maxCategoryInitAttempts})`,
      );
      setTimeout(initializeCategoryProducts, 500);
    } else {
      console.log(
        "âš ï¸ Category products initialization stopped after max attempts",
      );
    }
  }
}
...
function loadCategoryProducts(products) {
  ...
  Object.entries(categoryMappings).forEach(([gridId, categoryName]) => {
    ...
    const categoryProducts = products.filter(
      (p) => p.category === categoryName && p.showInSlider !== false,
    );
    ...
    console.log(
      ...
      products
        ...
        .map((p) => `${p.id}: ${p.name} (showInSlider: ${p.showInSlider})`),
    );
    console.log(
      `🔍 Filtered products:`,
      categoryProducts.map((p) => `${p.id}: ${p.name}`),
    );

    if (categoryProducts.length > 0) {
      console.log(
        `✅ Rendering ${categoryProducts.length} products to ${gridId}`,
      );
      ...
      console.warn(
        `⚠️ Total products in this category (including hidden):`,
  ...
  if (searchOverlay) {
    ...
    if (allProducts.length === 0) {
      loadProducts().then((products) => {
        ...
        console.log(
          'ðŸ"¦ Products loaded for category search:',
          allProducts.length,
        );
...
function filterAndDisplay(products, query, grid) {
  console.log(
    "ðŸ” filterAndDisplay called with:",
    products.length,
    "products, query:",
    `"${query}"`,
  );
