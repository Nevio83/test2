...
if (require.main === module) {
  console.log('🧪 TEST: CJ Payment Calculator\n');
  
  // Beispiel-Warenkorb
  const testCart = [
    { id: '10', name: 'Elektrischer Wasserspender', price: 28.99, quantity: 1 },
    { id: '11', name: '350ml Elektrischer Mixer', price: 24.99, quantity: 2 }
  ];
  
  console.log('📦 Test-Warenkorb:');
  testCart.forEach(item => {
    console.log(`   ${item.quantity}x ${item.name} - €${item.price}`);
  });
  
  showBreakdown(testCart, 'DE');
